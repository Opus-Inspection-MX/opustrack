...
- [x] 6.3 Drift-check validation — PROVEN user-side 2026-09-10: `npm run e2e:drift:proof` → PASS (green=0, negative=2), ephemeral shadow auto create-destroy. Wiring: `shadowDatabaseUrl` in schema, `scripts/drift-proof.mjs` (`db:drift:proof` / `e2e:drift:proof`, `--green-only` for `db:drift`)
