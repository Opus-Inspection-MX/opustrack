...
export function CreateIncidentDialog({
  ...
  const validateDateRange = (date: string, time: string): boolean => {
    ...
    if (selectedSchedule.endDate) {
      ...
      if (incidentDateTime < scheduleStart || incidentDateTime > scheduleEnd) {
        setDateRangeError(
          `La fecha del incidente debe estar entre ${scheduleStart.toLocaleString("es-MX", {
            dateStyle: "short",
            timeStyle: "short"
      ...
      if (incidentDateTime < scheduleStart) {
        setDateRangeError(
          `La fecha del incidente debe ser igual o posterior a ${scheduleStart.toLocaleString("es-MX", {
            dateStyle: "short",
            timeStyle: "short"
          })}`
        )
  ...
  }, [formData.scheduledDate, formData.scheduledTime, selectedSchedule])

  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault()

    // Validate date range if there's a selected schedule
    if (selectedSchedule && !validateDateRange(formData.scheduledDate, formData.scheduledTime)) {
      return
    }
  ...
  return (
    <ResponsiveDialog
      ...
      title={selectedSchedule ? "Nuevo Incidente en Programación" : "Nuevo Incidente Programado"}
      description={selectedSchedule ? `Crear incidente en la programación: ${selectedSchedule.title}` : "Crea un nuevo incidente con fecha de inicio programada"}
                ...
                <div className="flex-1">
                  ...
                  <div className="text-xs text-blue-600 dark:text-blue-400 mt-2 space-y-1">
                    <div>
                      ...
                      {new Date(selectedSchedule.scheduledAt).toLocaleString("es-MX", {
                        dateStyle: "short",
                        timeStyle: "short",
                      })}
                    </div>
                    {selectedSchedule.endDate && (
                      <div>
                        ...
                        {new Date(selectedSchedule.endDate).toLocaleString("es-MX", {
                          dateStyle: "short",
                          timeStyle: "short",
                        })}
                      </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="space-y-2">
              ...
              <Select
                ...
                onValueChange={(value) => setFormData({ ...formData, vicId: value })}
                required
              >
                <SelectTrigger>
          ...
          <div className="space-y-2">
            ...
            {dateRangeError && (
              <div className="bg-red-50 dark:bg-red-950 border border-red-200 dark:border-red-800 rounded-lg p-3">
                <p className="text-sm text-red-700 dark:text-red-300">{dateRangeError}</p>
              </div>
            )}
          </div>
