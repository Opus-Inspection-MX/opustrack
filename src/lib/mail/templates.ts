...
// ---------------------------------------------------------------------------
// Legacy per-event builders (deprecated).
//
// Kept only until `notify-events.ts` becomes a thin facade over `dispatch`
// (next commit): `dispatch` renders every event through the catalog above.
// Each wrapper preserves the exact legacy subject/body so the interim commit
...
/** @deprecated Render through `NOTIFICATION_EVENTS[type].render` instead. */
export function incidentCreatedEmail(
  ...
  return {
    ...
    body: [
      ...
      "",
      `Ábrelo aquí: ${legacyUrl(`/admin/incidents/${incidentId}`)}`,
      "",
      "— OpusTrack",
    ].join("\n"),
  };
...
export function incidentClosedEmail(
  ...
  return {
    ...
    body: [
      ...
      "",
      `Revísalo aquí: ${legacyUrl(`/admin/incidents/${incidentId}`)}`,
      "",
      "— OpusTrack",
    ].join("\n"),
  };
...
export function vacationRequestedEmail(
  ...
  return {
    ...
    body: [
      ...
      "",
      `Apruébala o recházala aquí: ${legacyUrl("/admin/vacations")}`,
      "",
      "— OpusTrack",
    ].join("\n"),
  };
