...
export default function VehicleStatusPage() {
  ...
  const actions: CatalogAction<VehicleStatus>[] = [
    ...
    },
    {
      ...
      onClick: async (row) => {
        try {
          const result = await deleteVehicleStatus(row.id);
          if (isFailure(result)) {
  ...
  return (
    <PageContainer>
      <PageHeader
        ...
        description="Gestionar tipos de estado de vehículo"
        actions={
          <Button
        ...
        mobileCard={(row) => (
          <div className="space-y-2 rounded-xl border bg-card p-4">
            ...
            <p className="text-xs text-muted-foreground">
              {row._count.vehicles} vehículos
            </p>
            <div className="flex gap-2">
              <Button asChild variant="outline" size="sm" className="flex-1">
                <Link href={`/admin/settings/vehicle-status/${row.id}`}>
                  Ver
                </Link>
              </Button>
              <Button asChild variant="outline" size="sm" className="flex-1">
                <Link href={`/admin/settings/vehicle-status/${row.id}/edit`}>
                  Editar
                </Link>
              </Button>
            </div>
