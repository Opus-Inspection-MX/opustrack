...
export default function EditIncidentTypePage({
  params,
}: {
  ...
  useEffect(() => {
    params.then((p) => setIncidentTypeId(parseInt(p.id, 10)));
  }, [params]);

  const fetchIncidentType = useCallback(async () => {
  ...
  }, [incidentTypeId, fetchIncidentType]);
